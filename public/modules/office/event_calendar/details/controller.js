myApp.registerCtrl('event_calendar_details_controller', ['$location', 'event_calendar_details_service', '$q', '$rootScope', '$scope', '$filter', function ($location, event_calendar_details_service, $q, $rootScope, $scope, $filter) {
    /**declare variable */
    ...
    /** init variable */
    {
        ...
        ctrl._urlProcess = FrontendDomain + "/modules/office/event_calendar/details/views/process.html";
        ...
        .then(
          function (res) {
            ...
            try{
              ...
              ctrl.Item = {
                ...ctrl.Item,
                ...getPermissionProperties(ctrl.Item),
                showRoom: $filter('showRoomEventCalendar')(ctrl.Item.meeting_room_registration),
                showCar: $filter('showCarEventCalendar')(ctrl.Item.vehicle_registration)
              }
            }catch(err){
              console.log(err);
            }
...
myApp.registerCtrl("event_calendar_management_detail_controller", [
  ...
  function (
    ...
    {
      ctrl._ctrlName = "event_calendar_management_detail_controller";
    ...
    function access_service(action) {
      return function () {
          ...
          $q.all(dfdAr).then(function () {
              if(action === 'creator_delete'){
                window.location.reload();
              }
              else{
                $rootScope.$broadcast('event_calendar::access');
              }
              $rootScope.statusValue.generate(ctrl._ctrlName, "EventCalendar", action);
